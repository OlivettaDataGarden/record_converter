...
import pytest
from data.test_data_process_command import BASE_PARAMS, PARAMS_ADD_COMMANDS, \
    PARAMS_ALLOW_NONE_VALUE_NONE, PARAMS_ALLOW_NONE_VALUE_WITH_VALUE, \
    PARAMS_CURRENT_YEAR, PARAMS_FIRST_ITEM_FROM_LIST, \
    PARAMS_FIRST_ITEM_FROM_LIST_NONE_RESULT, PARAMS_FROM_LIST, \
    PARAMS_FULL_RECORD, PARAMS_GET_FIXED_VALUE, PARAMS_INT_FROM_STRING, \
    PARAMS_JOIN, PARAMS_JOIN_ERROR, PARAMS_JOIN_KEY_VALUE, \
    PARAMS_JOIN_KEY_VALUE2, PARAMS_JOIN_KEY_VALUE_ERROR1, \
    PARAMS_JOIN_KEY_VALUE_ERROR2, PARAMS_JOIN_KEY_VALUE_ERROR3, \
    PARAMS_JOIN_WITH_FIXED_VALUE, PARAMS_JOIN_WITH_SEPERATOR, \
    PARAMS_JOIN_WITH_SEPERATOR_AND_NONE, PARAMS_NON_EXIST_COMM, \
    PARAMS_NORM_ADDRESS, PARAMS_NORM_ADDRESS_ERROR, PARAMS_POINT, \
    PARAMS_POINT_ERROR1, PARAMS_POINT_ERROR2, PARAMS_SET_TO_NONE_VALUE, \
    PARAMS_SPLIT_FIELD, PARAMS_SPLIT_FIELD_NONE, PARAMS_TO_INT_STRIP_LIST, \
    PARAMS_TO_INT_STRIP_STR, PARAMS_TO_LIST, PARAMS_TO_LIST_DYNAMIC
...
# def test_add_process_commands():
#     """Test to see if custom process commands can be added
#     """
#     processor = field_processor(PARAMS_ADD_COMMANDS)
#     assert processor.get_value() == 'fedcba'
...
# def test_norm_address(mocker):
#     """ test create an int from a string """
#     mocked_address = 'test123'
#     mocker.patch(
#         'record_converter.process.get_address.get_norm_address',
#         return_value=mocked_address)
#     processor = field_processor(PARAMS_NORM_ADDRESS)
#     assert processor.get_value() == mocked_address


# def test_norm_address_error1():
#     """ test create an int from a string """
#     processor = field_processor(PARAMS_NORM_ADDRESS_ERROR)
#     assert processor.get_value() is None
...
def test_point(mocker):
    ...
    mocker.patch(
        'record_converter.process.lat_lon_to_geojson_point',
...
def test_full_record():
    ...
    assert processor.get_value()['field1'] == \
        {'nested_field': {'a': 1}}
...
def test_to_list_dynamic():
    ...
    assert processor.get_value() == \
        [{'price1': '10.000 eur'}, {'price2': '9.000 eur'}]
