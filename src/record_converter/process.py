...
from record_converter.helper.geo_json import lat_lon_to_geojson_point
...
class ProcessCommand():
    ...
    def to_list(self):
        ...
        """
        return list(filter(
            None, [self._get_field(field_name) for field_name in
                   self.process_args]))
    ...
    def from_list(self):
        ...
        return list(filter(
            None, [self.record_convertor(
                rules=rules, record=obj).convert() for obj in obj_list]))

    def to_list_dynamic(self):
        return list(filter(
            None,
            [self.record_convertor(
                rules=rule, record=self.record).convert() for
             rule in self.process_args]))
    ...
    def join_key_value(self):
        ...
        if isinstance(key_key, dict):
            key = self.record_convertor(
                rules=key_key, record=self.record).convert()
        else:
            key = self._get_field(key_key)

        # check if value value needs to be composed
        if isinstance(value_key, dict):
            value = self.record_convertor(
                rules=value_key, record=self.record).convert()
        else:
    ...
    def point(self):
        ...
        return lat_lon_to_geojson_point(
            latitude=lat,
            longitude=lon
        )
    ...
    #     """
    #     Retrieves address in normalized format.

    #     args:
    #         value (dict):
    #             needs to contain keys address, zip_code, city and
    #             iso3116_country_code. If one of them misses None is returned

    #     returns:
    #         string: String with normalized address from geopy
    #     """
    #     args = self.process_args
    #     address = self._get_field(args.get('address', 'no field value'))
    #     zip_code = self._get_field(args.get('zip_code', 'no field value'))
    #     city = self._get_field(args.get('city', 'no field value'))
    #     country_code = self._get_field(
    #         args.get('iso3116_country_code', 'no field value'))

    #     try:
    #         full_address = ', '.join(
    #             [address, zip_code, city, country_code])
    #         return GeoAPI().get_normalized_address_from_address(full_address)
    ...
    def join(self):
        ...
        try:
            return seperator.join([
                join_value(key) for key in self.process_args]).strip()
        except KeyError:
            return None
    ...
    def get_value(self):
        ...
        cust_comm_class = self.add_process_commands.get(
            cust_command, None)
        ...
        raise NotImplementedError(
            f'Field conversion command `{self.process_command}`')
