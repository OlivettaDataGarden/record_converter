...
from record_converter.helper.html_parser import DataFromHTMLSnippet

from .helper.countries import iso3116_from_alpha_3_country_code
from .helper.string_helpers import normalize_string
...
class RecordFieldConvertor():
    ...
    def __init__(self, record, conversion_rule):
        self.record = record
        self.conversion_rule = conversion_rule
        self.field_name = conversion_rule.get('fieldname')
        if not self.field_name:
            raise ValueError('fieldname not provided in conversion rule')
        self.field_value = self._get_field(self.field_name)

    def multiply_by(self, action_value):
        """divides the given field by given value"""
        value_to_divde = self.field_value
        try:
            float_value = float(value_to_divde)
        except (ValueError, TypeError):
            return None

        return float_value * action_value

    def join_fields(self, action_value: list) -> str:
        """Joins values from a list of fields."""
        field_values = [
            self._get_field(field_name) for field_name in action_value]
    ...
    def alpha3_to_iso3116_cc(self, action_value):
        """converts a alpha 3 country code to a iso3116 country code"""
        return iso3116_from_alpha_3_country_code(
            self.field_value)
    ...
    def list_to_dict(self, action_value):
        ...
        return {
            normalize_string(item[0]).replace(' ', '_'):
    ...
    def add_key_value_from_field(self, action_value):
        ...
        return {
            key: self._get_field(key) for key in action_value}
    ...
    def add_data_from_list_of_dict(self, action_value):
        ...
        if not (key_key and value_key):
            raise KeyError(
                f'key_key {key_key} or value_key {value_key} missing')
        result = {}
    ...
    def convert(self):
        if self.all_conditions_true():
            for action_dict in self.conversion_rule['actions']:
    ...
    def all_conditions_true(self):
        """Returns True if all provided conditions are satisfied

        Returns:
            bool:
        """
        if conditions := self.conversion_rule.get('condition', False):
            return Conditions(
                provided_conditions=conditions,
                value=self.field_value).evaluate()
